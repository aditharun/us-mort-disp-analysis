...
ageadj <- ageadj %>% group_by(year, icd, gender) %>% summarize(ageadj_excess = ageadjrate[race=="black"] - ageadjrate[race=="white"]) %>% ungroup()
...
pct_deaths <- rbind(ageadj, (covid_excess_aamrs %>% mutate(year = 2020, name = c("COVID", "COVID")) %>% dplyr::rename(gender = Gender))) %>% group_by(year, gender) %>% summarize(net = sum(ageadj_excess)) %>% ungroup() %>% left_join(net_excess_ageadj, by=c("gender"="gender", "year"="year")) %>% mutate(pct_captured = round((net*100) / excess_deaths_rate)) 
...
ageadj <- ageadj %>% left_join(name_map, by=c("name"="name")) %>% select(-name) %>% magrittr::set_colnames(c("year", "gender", "ageadj_excess", "name"))
...
top_half_fig1 <- plot_grid(fig1a, right_half_fig1, nrow = 1, labels = c("A", ""), rel_widths = c(1.25, 1), label_size = 20)

ggsave(plot = top_half_fig1, filename = "results/decline-fig1.pdf", device = cairo_pdf, height  = 6, width = 15)


saveRDS(object = ageadj, file = "results/yearly-aamrs.rds")
...
#Overall Excess AAMR Decline (1999-2011)
...
fig2a <- m_avg_rate  %>% ggplot( aes(y = name, x = slope, fill = plateau)) +
    ...
    theme_minimal() + theme(panel.border = element_rect(color = "black", fill = "transparent"), panel.grid = element_blank()) + geom_vline(xintercept = 0, color = "grey30") + theme(panel.grid.major.y = element_line(color = "grey90", linetype = "dashed", linewidth = 0.5)) + scale_fill_manual(values = c("Decline" = cpal[1], "Non-decline" = cpal[2]), labels = c("Decline" = "1999-2011", "Non-decline" = "2012-2019"), guide = guide_legend(nrow = 2)) + ggtitle("Males") + theme(plot.title = element_text(size = 14, hjust = 0.5)) + theme(legend.position = "bottom") + theme(axis.text = element_text(size = 12), axis.title = element_text(size = 14), legend.text = element_text(size = 12)) + xlim(-5, 5)
...
fig2b <- f_avg_rate %>% ggplot( aes(y = name, x = slope, fill = plateau)) +
    geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.75) +
    labs(y = "", x = "Average Change in Annual Excess AAMR", fill = "")  + geom_text(aes(y=name, x=4, label = paste0("(", abs_change, ", ", pct_change, "%)")), size = 3.5) + 
    theme_minimal() + theme(panel.border = element_rect(color = "black", fill = "transparent"), panel.grid = element_blank()) + geom_vline(xintercept = 0, color = "grey30") + theme(panel.grid.major.y = element_line(color = "grey90", linetype = "dashed", linewidth = 0.5)) + scale_fill_manual(values = c("Decline" = cpal[1], "Non-decline" = cpal[2]), labels = c("Decline" = "1999-2015", "Non-decline" = "2016-2019"), guide = guide_legend(nrow = 2)) + ggtitle("Females") + theme(plot.title = element_text(size = 14, hjust = 0.5)) + theme(legend.position = "bottom") + theme(axis.text = element_text(size = 12), axis.title = element_text(size = 14), legend.text = element_text(size = 12)) + xlim(-5, 5)
